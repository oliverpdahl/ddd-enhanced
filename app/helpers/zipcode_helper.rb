module ZipcodeHelper
    ...
    def getZipcodeAccidentGrowthByQuarterQuery(zipcode)
        ...
        TRUNC(EXTRACT(MONTH FROM accident_all.Start_Time)/4) + 1 "Quarter", 
        CONCAT(TRUNC(EXTRACT(MONTH FROM accident_all.Start_Time)/4) + 1, CONCAT(\'-\', EXTRACT(YEAR FROM accident_all.Start_Time))) "Qt-Year",
        ...
        TRUNC(EXTRACT(MONTH FROM accident_all.Start_Time)/4) + 1, "Most_Populous".population ORDER BY EXTRACT(YEAR FROM accident_all.Start_Time), 
        TRUNC(EXTRACT(MONTH FROM accident_all.Start_Time)/4) + 1, "Most_Populous".zipcode'
